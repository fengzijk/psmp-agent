...
# COPY 源路径 目标路径
COPY . $GOPATH/src/fengzijk.com/psmp-agent

# RUN 执行 go build .
RUN cd $GOPATH/src/fengzijk.com/psmp-agent && go mod tidy && go build -o server .
...
COPY --from=builder /opt/repo/src/fengzijk.com/psmp-agent /opt
